import { ShirtsTshirts } from "../../lib/assets";
...
export const getStaticPaths = async () => {
  ...
  const pageTshirt = Tshirts.map((item) => ({
...
export const getStaticProps = async ({ params }) => {
  ...
  const dataDetailTshirt = Tshirts.filter((d) => d.id.toString() === params.id);
  return {
    props: {
      data: dataDetail[0],
      dataTshirt: dataDetailTshirt[0],
    },
  };
};
